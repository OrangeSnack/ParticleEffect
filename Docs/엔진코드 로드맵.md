...
- OnDestroy() 이벤트 호출 시간에 무조건 Object로 구현한 Component가 살아있어야 함으로 ObjectManager나 어떤 레지스트리도 괜찮으니 지연파괴를 구현할 것, 그래서 Object를 삭제할 때 개념상으로만 삭제시키고 실제 로직에서는 프레임 처리 맨 마지막에 삭제가 필요한 Object를 정리할 수 있도록 보장시켜야 함
